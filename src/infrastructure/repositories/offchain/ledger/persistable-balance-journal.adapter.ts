...
import { type Balance } from '../../../../domain/ledger/entities/balance.entity';
import { type JournalEntry } from '../../../../domain/ledger/entities/journal-entry.entity';
import { type PersistableBalanceJournalPort } from '../../../../domain/ledger/ports/persistable-balance-journal.port';
  ...
  implements PersistableBalanceJournalPort
{
  ...
  async save(balance: Balance, journalEntry: JournalEntry): Promise<void> {
    ...
    try {
      await this.db().raw(
        `insert into 
          balance (account, "group", total, l1, l2, status, nonce, join_date, exit_date, last_journal_entry_date, last_journal_movement_type, last_journal_entry_amount, last_journal_entry_chain_id, checksum, uint256_total)
          values (:account, :group, :total, :l1, :l2, :status, :nonce, :joinDate, :exitDate, :lastJournalEntryDate, :lastJournalMovementType, :lastJournalEntryAmount, :lastJournalEntryChainId, :checksum, :uint256Total)
          on conflict (account, "group")
          do update 
          set total = :total, l1 = :l1, l2 = :l2, status = :status,
          nonce = :nonce, join_date = :joinDate, exit_date = :exitDate,
          last_journal_entry_date = :lastJournalEntryDate,
          last_journal_movement_type = :lastJournalMovementType,
          last_journal_entry_amount = :lastJournalEntryAmount,
          last_journal_entry_chain_id = :lastJournalEntryChainId,
          checksum = :checksum, uint256_total = :uint256Total, updated_at = :updatedAt;`,
        param,
      );
    } catch (err) {
      console.log(err);
    ...
    await this.db().raw(
      `insert into
      journal (chain_id, block_number, transaction_hash, amount, uint256_amount, movement_type, entry_type, account, account_group, iso_code, log_index, entry_date, gas_used, cumulative_gas_used, effective_gas_price)
      values (:chainId, :blockNumber, :transactionHash, :amount, :uint256Amount, :movementType, :entryType, :account, :accountGroup, :isoCode, :logIndex, :entryDate, :gasUsed, :cumulativeGasUsed, :effectiveGasPrice);`,
      journalParam,
    );
  }
}
