...
import { type OnChainUserReceipt } from '../../../../domain/supply/dtos/onchain-user-receipt.dto';
import { type PersistableClaimReceiptPort } from '../../../../domain/transaction/ports/persistable-claim-receipt.port';
  ...
  implements PersistableClaimReceiptPort
{
  ...
  async create(order: Order, userReceipt: OnChainUserReceipt): Promise<void> {
    ...
    await this.db().raw(
      `insert into 
        "user_receipt" (chain_id, block_number, transaction_hash, order_id, "from", "to", gas_used, cumulative_gas_used, effective_gas_price, amount_in_knn, user_identifier, payment_id, created_at)
        values (:chainId, :blockNumber, :transactionHash, :orderId, :from, :to, :gasUsed, :cumulativeGasUsed, :effectiveGasPrice, :amountInKnn, :userIdentifier, (select id from "payment" as p where p.sequence = :paymentSequence), :createdAt);`,
      userReceiptParam,
    );

    await this.db().raw(
      `insert into 
        "claim" (id, payment_id, onchain_address, order_id, uint256_amount, created_at, user_transaction_hash, updated_at)
        values (:id, (select id from "payment" as p where p.sequence = :paymentSequence), :onchainAddress, :orderId, :uint256Amount, :createdAt, :userTransactionHash, :updatedAt)`,
      claimParam,
    );
  }
}
