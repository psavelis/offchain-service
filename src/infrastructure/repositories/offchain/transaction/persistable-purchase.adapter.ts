...
import { type PersistablePurchasePort } from '../../../../domain/transaction/ports/persistable-purchase.port';
...
export class PersistablePurchaseDbAdapter implements PersistablePurchasePort {
  ...
  async create({
    ...
    await this.db().raw(
      `insert into
      purchase (payment_date, total_knn, knn_price_in_usd, total_usd, total_gas_usd, contract_address, 
        network, crypto_wallet, purchase_transaction_hash, total_eth, total_gas_eth, eth_price_in_usd, ethereum_block_number, total_matic, total_gas_matic, matic_price_in_usd, polygon_block_number)
      values (:paymentDate, :totalKnn, :knnPriceInUsd, :totalUsd, :totalGasUsd, :contractAddress, :network, :cryptoWallet, 
        :purchaseTransactionHash, :totalEth, :totalGasEth, :ethPriceInUsd, :ethereumBlockNumber, :totalMatic, :totalGasMatic, :maticPriceInUsd, :polygonBlockNumber);`,
      param,
    );
  }
}
