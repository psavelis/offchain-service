...
import {type FetchableJournalTransferEventPort} from '../../../../domain/ledger/ports/fetchable-journal-transfer-event.port';
import {
  AccountGroup,
  JournalEntry,
  JournalEntryType,
...
import {type TransferEvent} from '../protocol/@openzeppelin/contracts/token/ERC20/ERC20';
    ...
    const purchases = Promise.all(
      rawEvents.map(async (event) => {
        ...
        return event
          ...
          .then(async ([credit, debit]: JournalEntry[]) => {
            return event
              .getBlock()
              .then((block) => {
