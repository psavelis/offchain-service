...
import {type ClaimLockedSupplyDto} from '../../../../domain/supply/dtos/claim-locked-supply.dto';
...
import {type KannaDynamicPriceSale, type KannaPreSale} from '../protocol/contracts';
...
export class DelegateClaimRpcAdapter implements DelegateClaimPort {
  ...
  async delegateClaimLocked(
    ...
    if (!signer) {
      ...
      } (SettledChainId: ${order.getSettledChainId()}, ConnectedSigners: ${JSON.stringify(
        Object.keys(this.signerHashMap[chain.layer]).map((key) => ({
          [key]: SignerType[this.signerHashMap[chain.layer][key]],
        })),
      )}`;
